...
$nuspecPath = $(Join-Path $(Get-Location) ..\ReMi.Core\.build)
$nuspecName = "ReMi.Core.nuspec"
$nugetConfig = $(Join-Path $(Get-Location) ..\.nuget\NuGet.config)
$outputFolder = $(Join-Path $nuspecPath "lib\\net45")
$matchPattern = ("ReMi.Contracts.dll")

$version = GetNextVersion $nuspecName $nuspecPath $nugetRepo

ChangeVersionInAssemblies $version ..\

PublishToFileSystem $(Join-Path $nuspecPath ..\ReMi.Contracts\ReMi.Contracts.csproj) $outputFolder
