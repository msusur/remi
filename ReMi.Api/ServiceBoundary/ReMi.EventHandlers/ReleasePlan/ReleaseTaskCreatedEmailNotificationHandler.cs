...
{
    public class ReleaseTaskCreatedEmailNotificationHandler : IHandleEvent<ReleaseTaskCreatedEvent>
    {
        ...
        public void Handle(ReleaseTaskCreatedEvent evnt)
        {
            if (evnt == null)
            ...
            if (releaseTask.CreatedByExternalId == null || releaseTask.CreatedByExternalId == Guid.Empty)
                throw new ArgumentException(String.Format("Creator of release task is missing. ReleaseTask={0}", releaseTask));
        ...
        private void NotifyAssignee(ReleaseTask releaseTask, ReleaseWindow releaseWindow, Account assignee)
        {
            var replaceValues = new Dictionary<string, object>
            {
                ...
                {"Creator", releaseTask.CreatedBy},
                {"HelpDeskUrl", releaseTask.CreateHelpDeskTicket ? String.Format("<a href=\"{0}\">#{1}</a>",
                    releaseTask.HelpDeskTicketUrl, releaseTask.HelpDeskTicketReference) : "not created"},
            ...
            const string subject = "{0} Release task created";

            var assigneeEmail = EmailTextProvider.GetText("TaskWasAssignedEmail", replaceValues);
            EmailService.Send(assignee.Email,
                string.Format(subject, releaseWindow.Products.FormatElements(string.Empty, string.Empty)),
                assigneeEmail);
        }
        ...
        private void NotifySupportSubscribers(ReleaseTask releaseTask, ReleaseWindow releaseWindow, Account assignee)
        {
            ...
            foreach (var recipient in accounts)
            {
                ...
                var assigneeEmail = EmailTextProvider.GetText("TaskWasCreatedSupportMembersEmail", replaceValues);
                EmailService.Send(
                    recipient.Email,
                    string.Format("{0} Release task created", releaseWindow.Products.FormatElements(string.Empty, string.Empty)),
                    assigneeEmail);
            }
        }
